...
impl Lake {
    ...
    pub fn delete_webhook(&self, hook_id: &str) -> Result<usize> {
        // Soft-disable — same DuckDB 1.1 PRIMARY KEY RemoveFromIndexes constraint as schedules.
        let conn = self.conn.lock().unwrap();
        let n = conn.execute(
            "UPDATE webhooks SET enabled=false WHERE hook_id=?",
    ...
    pub fn upsert_schedule(&self, s: &Schedule) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        // DuckDB 1.1 crashes on DELETE+INSERT against a PRIMARY KEY index
        // (RemoveFromIndexes fatal). Use UPDATE-then-INSERT-if-missing instead.
        let updated = conn.execute(
            "UPDATE schedules
             SET cron_expr=?, project=?, assigned_agent=?, skill=?, model=?,
                 payload=?, priority=?, enabled=?, next_run_at=?,
    ...
    pub fn delete_schedule(&self, id: &str) -> Result<usize> {
        // Soft-disable instead of DELETE — DuckDB 1.1 PRIMARY KEY index triggers
        // RemoveFromIndexes fatal on DELETE. Setting enabled=false is equivalent:
        // due_schedules() filters on enabled=true so it will never fire again.
        let conn = self.conn.lock().unwrap();
        let n = conn.execute(
            "UPDATE schedules SET enabled=false WHERE id=?",
            duckdb::params![id],
        )?;
        Ok(n)
    }
