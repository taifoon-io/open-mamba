/// Returns just the CREATE TABLE statement for task_envelopes (no IF NOT EXISTS).
/// Used by purge_corrupted_rows after DROP TABLE to rebuild a clean table.
pub fn task_envelopes_create_sql() -> &'static str {
    r#"CREATE TABLE task_envelopes (
    id              UUID        NOT NULL,
...
-- No indexes — same DuckDB 1.1 RemoveFromIndexes bug applies to all index types."#
}

/// DDL for all DuckDB tables. Run once on startup (idempotent).
pub const INIT_SQL: &str = r#"
-- ── Task envelopes ────────────────────────────────────────────────────────────
-- NOTE: id is NOT PRIMARY KEY — DuckDB 1.1 has a fatal RemoveFromIndexes bug
-- that crashes the process on any DELETE/UPDATE against a PK-indexed UUID table.
-- Uniqueness is guaranteed by UUID generation. We use a regular UNIQUE index below.
...
-- Idempotent column adds for existing DBs upgraded from older schema versions.
ALTER TABLE task_envelopes ADD COLUMN IF NOT EXISTS response    TEXT;
ALTER TABLE task_envelopes ADD COLUMN IF NOT EXISTS retry_count INTEGER DEFAULT 0;
ALTER TABLE task_envelopes ADD COLUMN IF NOT EXISTS not_before  TIMESTAMPTZ;
-- NOTE: No indexes on task_envelopes — DuckDB 1.1 has a fatal RemoveFromIndexes
-- assertion bug triggered by DELETE/UPDATE on any indexed column, including
-- PRIMARY KEY and UNIQUE indexes. With only 100s-1000s of rows, full scans
-- are acceptable. Re-add indexes when upgrading to DuckDB 1.2+.
...
-- NOTE: No PRIMARY KEY and no secondary index — same DuckDB 1.1 RemoveFromIndexes
-- fatal bug applies here. The advance_schedule UPDATE on next_run_at would crash
-- the process if idx_schedules_due existed. Full scans on a small table are fine.
