...
//! - http-forward:<url> → raw HTTP POST of the payload to the target URL
...
impl Bus {
    ...
    async fn route(&self, envelope: TaskEnvelope) -> Result<()> {
        ...
        } else if envelope.model.starts_with("http-forward:") {
            let url = envelope.model.trim_start_matches("http-forward:").to_string();
            self.dispatch_http_forward(envelope, url).await
        } else {
            self.dispatch_openfang(envelope).await
        }
    }

    /// POST the task payload directly to a target URL.  Used for
    /// integrations (like the LWC delivery worker) that don't need an
    /// AI agent — just a reliable scheduled HTTP trigger.
    async fn dispatch_http_forward(&self, envelope: TaskEnvelope, url: String) -> Result<()> {
        info!(id = %envelope.id, url = %url, "http-forward dispatch");
        ...
        let payload: serde_json::Value = serde_json::from_str(&envelope.payload)
            .unwrap_or_else(|_| serde_json::json!({ "payload": envelope.payload }));
        ...
        match client.post(&url).json(&payload).send().await {
            Ok(resp) => {
                ...
                info!(id = %envelope.id, http_status = status, "http-forward ok");
                self.lake.complete_envelope(envelope.id, 0, 0, 0.0, Some(body.as_str()))?;
            }
            Err(e) => {
                let msg = e.to_string();
                error!(id = %envelope.id, "http-forward failed: {msg}");
                let is_transient = is_transient_error(&msg);
                self.lake.record_dispatch_failure(
                    envelope.id, is_transient, 2, if is_transient { 60 } else { 0 },
                )?;
                if !is_transient {
                    anyhow::bail!("http-forward permanent failure: {msg}");
                }
            }
        }
        Ok(())
    ...
    async fn dispatch_nemotron(&self, envelope: TaskEnvelope, adapter: NemotronAdapter) -> Result<()> {
        ...
        self.lake.update_status(envelope.id, TaskStatus::Running, None)?;
        info!(id = %envelope.id, adapter = %adapter.slug(), "nemotron: status=running, starting generate");

        let system_str = adapter_system_prompt(adapter);
        // General adapter resolves system prompt from env internally — pass None.
        let system_opt = if system_str.is_empty() { None } else { Some(system_str) };
        let gen_result = self.nemotron.generate(adapter, &envelope.payload, system_opt, Some(512)).await;
        info!(id = %envelope.id, adapter = %adapter.slug(), "nemotron: generate returned");
        ...
        match gen_result {
            Ok(resp) => {
                ...
                let _ = self.lake.save_response(envelope.id, &resp.response);
                self.lake.complete_envelope(envelope.id, 0, tokens_out, cost_usd, None)?;
                if let Ok(record) = self.billing.process(&envelope, 0, tokens_out, cost_usd).await {
                    let _ = self.lake.insert_billing(&record);
                }
                Ok(())
            }
            Err(e) => {
                let msg = e.to_string();
                info!(id = %envelope.id, "nemotron: calling record_dispatch_failure, err={msg}");
                let is_transient = is_transient_error(&msg);
                let rescheduled = self
                    .lake
                    .record_dispatch_failure(envelope.id, is_transient, 2, if is_transient { 60 } else { 0 })
                    .unwrap_or(false);
                info!(id = %envelope.id, "nemotron: record_dispatch_failure returned rescheduled={rescheduled}");
                if rescheduled {
                    info!(id = %envelope.id, "nemotron transient failure, rescheduled: {msg}");
                } else {
                    error!(id = %envelope.id, "nemotron dispatch failed permanently: {msg}");
                }
                Err(e)
            }
        }
...
fn adapter_system_prompt(adapter: NemotronAdapter) -> &'static str {
    match adapter {
        NemotronAdapter::Taifoon    => "You are Taifoon Intel — protocol routing, V5 finality, solver economics.",
        NemotronAdapter::Polymarket => "You are Nemotron-Enhanced — 15-minute crypto price predictions on Polymarket.",
        NemotronAdapter::Algotrada  => "You are AlgoTrada Brain, an elite institutional trading system. Analyze market setups using DNA patterns, SMT divergence, Power of 3, and Goldbach zones. Output trading decisions as JSON.",
        // General: system prompt is resolved inside NemotronClient::generate from
        // NEMOTRON_GENERAL_SYSTEM env. Returning empty string here triggers that path.
        NemotronAdapter::General    => "",
    }
}
