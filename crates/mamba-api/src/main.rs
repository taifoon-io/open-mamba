...
// Use a larger thread pool so concurrent long-running agent turns
// (cargo build, multi-file edits) don't starve the HTTP handler.
// Default is num_cpus; 16 ensures axum can always serve /api/tasks
// even when WORKER_CONCURRENCY slots are fully occupied.
